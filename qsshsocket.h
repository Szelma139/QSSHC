...
public:void setOperationNotExecuted(bool value);
    enum SshError
    {
...
private:
    ...
    int numOfFiles = 1;
    int m_connectionSuccess=0;
    int m_loginSuccess = 0;
    QStringList commandResult;
    bool currentDownloadFinished;
    QTimer * timer;
public:
    bool returnLoggedIn();
    bool getSessionOpen();
    QString getElementsOfList(QStringList list);
    QStringList getCommandResult();
    int getConnectionSuccess();
    void setConnectionSuccess(int value);

    int getLoginSuccess();
    void setLoginSuccess(int loginSuccess);


    QString getFileResultInfo() const;
    void setFileResultInfo(const QString &value);
